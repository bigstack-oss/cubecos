...
license_check()
{
    ...
    local days
    days=$(/usr/sbin/hex_config license_check $app)
    local r=$?
    if [ "$VERBOSE" == "1" ]; then
...
license_import_check()
{
    ...
    if [ -f $dir/$name.license ]; then
        ...
        local curr_days
        curr_days=$(/usr/sbin/hex_config license_check $app)
        local curr_ret=$?

        local new_days
        new_days=$(/usr/sbin/hex_config license_check $app /var/support/$name)
        local new_ret=$?
...
license_cluster_import_check()
{
    ...
    do
        ...
        if [ "$node" == "$(hostname)" ]; then
            /usr/sbin/hex_sdk license_import_check $dir $name
        else
            host_remote_run $node /usr/sbin/hex_sdk license_import_check $dir $name
        fi
        local r=$?
...
license_cluster_check()
{
    ...
    do
        local node=$(echo $node_entry | head -c -1)
        if [ "$node" == "$(hostname)" ]; then
            /usr/sbin/hex_sdk -v license_check $app
        else
            host_remote_run $node /usr/sbin/hex_sdk -v license_check $app
        fi
    done
}
...
license_cluster_show()
{
    ...
    do
        ...
        if ping -c 1 -w 1 $node >/dev/null; then
            local item=$(host_remote_run $node /usr/sbin/hex_sdk $opts license_show $app)
            if [ "$FORMAT" == "json" ]; then
                if [ -n "$output" ]; then
                    output="$output, $item"
                else
...
license_cluster_import()
{
    ...
    if [ -f $dir/$name.license ]; then
        ...
        do
            ...
            if [ "$node" == "$(hostname)" ]; then
                mkdir -p $LICENSE_DIR
                /usr/sbin/hex_config license_check $app /var/support/$name >/dev/null
                ...
                host_remote_run $node /usr/sbin/hex_config license_check $app /var/support/$name >/dev/null
                if [ $? -eq $LICENSE_VALID ]; then
...
license_cluster_serial_get()
{
    ...
    do
        local node=$(echo $node_entry | head -c -1)
        local serial=$(host_remote_run $node /usr/sbin/hex_sdk license_serial_get)
        if [ -n "$serials" ]; then
            hosts="$hosts,$node"
