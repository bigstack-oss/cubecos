...
security_flow_abnormal_detection_run()
{
    ...
    do
        for d in "${direction[@]}"
        do
            for t in $(influx -host $(shared_id) -format json -database telegraf -execute "select tenant_name,last(bytes) from "hc"."sflow" where time >= now() - $base group by tenant_name" | jq -r .results[].series[].tags.tenant_name 2>/dev/null)
            do
                cgexec -g cpu:flowdetect hex_sdk security_tenant_flow_abnormal_detection $t $p $d $base $unit $threshold &
            done

            for i in $(influx -host $(shared_id) -format json -database telegraf -execute "select instance_id,last(bytes) from "hc"."sflow" where time >= now() - $base group by instance_id" | jq -r .results[].series[].tags.instance_id 2>/dev/null)
            do
...
security_flow_abnormal_detection()
{
    ...
    for c in $(cubectl node -r control list -j | jq -r .[].hostname)
    do
