...
for fn in gpu_device_profile_ensure os_device_profile_name_for os_device_profile_groups_for ; do
...
# sysfs stand-in: the function reads /sys/bus/pci/devices/<addr>/device, so the
# test points gpu_sysfs_pci_addr at a fake tree instead of a real address.
SYSFS="$TMP/sys"; mkdir -p "$SYSFS/0000:86:00.0"
echo "0x2531" > "$SYSFS/0000:86:00.0/device"
MOCK_ADDR="$SYSFS/0000:86:00.0"
gpu_sysfs_pci_addr() { printf '%s' "$MOCK_ADDR"; }
# rewrite the one absolute path the function uses onto the fake tree
eval "$(declare -f gpu_device_profile_ensure | sed 's#"/sys/bus/pci/devices/$addr/device"#"$addr/device"#')"
...
MOCK_NAMES="" MOCK_ADDR=""
gpu_device_profile_ensure GPU-a >/dev/null 2>&1;   ck "$?" "1" "no PCI address -> non-zero"
MOCK_ADDR="$SYSFS/0000:86:00.0"

rm -f "$MOCK_ADDR/device"
gpu_device_profile_ensure GPU-a >/dev/null 2>&1;   ck "$?" "1" "unreadable product id -> non-zero"
echo "0x2531" > "$MOCK_ADDR/device"
