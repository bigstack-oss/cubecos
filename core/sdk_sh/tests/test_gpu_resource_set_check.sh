...
# `virsh list` succeeding and a per-domain `dumpxml` then failing is its own
# case: the domain may have gone away (not holding) or libvirtd may have
# stopped answering mid-loop (unknown), and the exit status is the same. Piping
# dumpxml straight into grep made the pipeline report grep's status, so either
# one read as "this domain does not hold the card". The stub has no XML for a
# domain named "missing", so cat fails exactly as virsh would.
VIRSH_LIST="missing"
gpu_pgpu_holding_domain 00000000:8C:00.0 >/dev/null 2>&1
ck "$?" "1" "a dumpxml that fails is 'cannot tell', not 'not holding'"

VIRSH_LIST="missing holder"
gpu_pgpu_holding_domain 00000000:8C:00.0 >/dev/null 2>&1
ck "$?" "1" "...and it refuses there rather than walking on to the next domain"

# It must not over-refuse either: a failure after the answer is already in hand
# changes nothing.
VIRSH_LIST="holder missing"
ck "$(gpu_pgpu_holding_domain 00000000:8C:00.0)" "holder" \
   "a dumpxml failure after the holder was found does not unmake the answer"
