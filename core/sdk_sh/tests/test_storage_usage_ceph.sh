...
SRVSTO=10
CEPH="ceph"
for fn in storage_usage_ceph_disks _storage_usage_promql storage_usage_ceph_pools ; do
...
Debug(){ :; }                 # sdk logging helper, absent outside hex_sdk
# storage.usage.exact is read through hex_tuning, a script the caller sources
SETTINGS_TXT=$(mktemp); : > $SETTINGS_TXT
TUNEBIN=$(mktemp -d); PATH="$TUNEBIN:$PATH"
: > $TUNEBIN/hex_tuning; chmod +x $TUNEBIN/hex_tuning

# cinder-volumes: one image with a snapshot. ephemeral-vms: one image whose
# fast-diff map is invalid. Other pools empty.
# the function under test runs inside $( ), so the mock records what it saw in
# a file rather than a variable -- a subshell's assignments do not come back
RBD_LOG=$(mktemp /tmp/rbdlog.XXXXXX)
RBD_EXACT_FAILS=""
rbd(){
    local pool="" a saw=""
    for a in "$@" ; do [ "$a" = "--exact" ] && saw=yes ; done
    echo "${saw:-plain}" >> "$RBD_LOG"
    # an exact scan that outruns the timeout must not lose the pool's rows
    if [ -n "$RBD_EXACT_FAILS" -a -n "$saw" ] ; then return 124 ; fi
...
# --exact is the point of the reading: the object map counts an object at its
# full size once anything is written into it, and a discard empties an object
# without removing it from the map, so the map reports a high-water mark that
# never falls. Measured on a lab volume: 34G by the map against 1.5G exact.
chk "exact by default"    "$(grep -c '^yes$' "$RBD_LOG")"               "4"
chk "never plain by dflt" "$(grep -c '^plain$' "$RBD_LOG")"             "0"

: > "$RBD_LOG"
printf 'T_storage_usage_exact=false\n' > $TUNEBIN/hex_tuning
out2="$(storage_usage_ceph_disks)"
: > $TUNEBIN/hex_tuning
chk "tunable turns it off" "$(grep -c '^yes$' "$RBD_LOG")"              "0"
chk "still reports rows"  "$(echo "$out2" | grep -c .)"                 "2"

# a pool too large to scan inside the timeout falls back to the map rather
# than dropping every disk row, and says so through confidence
: > "$RBD_LOG"; RBD_EXACT_FAILS=1
out3="$(storage_usage_ceph_disks)"
RBD_EXACT_FAILS=""
chk "degrade retries"     "$(grep -c '^plain$' "$RBD_LOG")"             "4"
chk "degrade keeps rows"  "$(echo "$out3" | grep -c .)"                 "2"
chk "degrade is flagged"  "$(echo "$out3" | jq -r 'select(.disk_id=="volume-aaa") | .confidence')" "estimated"
...
# ceph-mgr already exports these as ceph_pool_*, so the function reads
# prometheus first and only falls back to `ceph df detail` when that is
# unavailable -- a capacity alarm must not go quiet because the metrics stack
# is the thing that broke. Both paths must produce the same record.
PROM_FAILS=""
CURL="curl"
curl(){
    [ -n "$PROM_FAILS" ] && return 1
    case "$*" in
    *ceph_pool_metadata*)
        echo '{"data":{"result":[
          {"metric":{"pool_id":"7","name":"cinder-volumes"},"value":[0,"1"]},
          {"metric":{"pool_id":"9","name":"glance-images"},"value":[0,"1"]}]}}' ;;
    *ceph_pool_*)
        echo '{"data":{"result":[
          {"metric":{"__name__":"ceph_pool_stored","pool_id":"7"},"value":[0,"100000000000"]},
          {"metric":{"__name__":"ceph_pool_bytes_used","pool_id":"7"},"value":[0,"300000000000"]},
          {"metric":{"__name__":"ceph_pool_max_avail","pool_id":"7"},"value":[0,"500000000000"]},
          {"metric":{"__name__":"ceph_pool_compress_under_bytes","pool_id":"7"},"value":[0,"40000000000"]},
          {"metric":{"__name__":"ceph_pool_compress_bytes_used","pool_id":"7"},"value":[0,"10000000000"]},
          {"metric":{"__name__":"ceph_pool_stored","pool_id":"9"},"value":[0,"10000000000"]},
          {"metric":{"__name__":"ceph_pool_bytes_used","pool_id":"9"},"value":[0,"30000000000"]},
          {"metric":{"__name__":"ceph_pool_max_avail","pool_id":"9"},"value":[0,"500000000000"]},
          {"metric":{"__name__":"ceph_pool_compress_under_bytes","pool_id":"9"},"value":[0,"0"]},
          {"metric":{"__name__":"ceph_pool_compress_bytes_used","pool_id":"9"},"value":[0,"0"]}]}}' ;;
    esac
}
...
# with prometheus unreachable the ceph df path must yield the same record
PROM_FAILS=1
fout="$(storage_usage_ceph_pools)"
PROM_FAILS=""
fget(){ echo "$fout" | jq -r --arg p "$1" 'select(.pool==$p) | '"$2"; }
chk "fallback pool rows"     "$(echo "$fout" | grep -c .)"                     "2"
chk "fallback stored"        "$(fget cinder-volumes .stored_bytes)"            "100000000000"
chk "fallback raw used"      "$(fget cinder-volumes .raw_used_bytes)"          "300000000000"
chk "fallback replication"   "$(fget cinder-volumes .replication_factor)"      "3"
chk "fallback reduction"     "$(fget cinder-volumes .reduction_ratio)"         "4"
chk "fallback null compr"    "$(fget glance-images .reduction_ratio)"          "null"

rm -rf "$RBD_LOG" "$SETTINGS_TXT" "$TUNEBIN"
