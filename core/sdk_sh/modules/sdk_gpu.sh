...
gpu_pgpu_holding_domain()
{
    ...
    # The XML is read into a variable rather than piped straight into grep: a
    # pipeline's exit status is grep's, so a dumpxml that fails would arrive
    # here as "this domain does not hold the card". It can fail because the
    # domain went away between the listing and now - which is a correct "not
    # holding" - or because libvirtd stopped answering mid-loop, which is not.
    # Neither is distinguishable from the other's exit status, and only one of
    # them is safe to guess, so both are "cannot tell".
    local vm_id xml
    for vm_id in $domains; do
        xml=$(virsh dumpxml "$vm_id" 2>/dev/null) || return 1

        if printf '%s' "$xml" | grep -qF "$pattern"; then
            echo "$vm_id"
            return 0
        fi
    done
