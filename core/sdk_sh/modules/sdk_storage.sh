...
storage_usage_ceph_disks()
{
    ...
    # --exact reads every object rather than trusting the object map. The map
    # counts an object at its full 8M once anything has been written into it,
    # and a discard empties an object without removing it from the map, so a
    # volume that has been written across and then trimmed keeps reporting the
    # high-water mark forever. Measured on a lab volume: 34G by the map, 1.5G
    # exact, against 11.4G that ceph df says the whole pool holds. Costs about
    # 18x -- 6s versus 0.3s for a 709G pool -- which is why it is a tunable, not
    # a constant: a cluster with millions of objects can turn it off and accept
    # the high-water mark.
    exact=""
...
# Instant query against the node's prometheus, returning the result array.
# The API lives under the route prefix its --web.external-url sets; the bare
# path answers 404, which reads exactly like "no data".
...
storage_usage_ceph_pools()
{
    ...
    # ceph-mgr already exports every figure here as ceph_pool_*, scraped into
    # prometheus as job "ceph". Read that rather than recomputing from a second
    # `ceph df detail`: one producer, one set of numbers. The metadata series
    # carries the pool name, which the value series does not -- joining them in
    # PromQL needs a group_left that this data does not match uniquely, so the
    # two are fetched separately and joined here on pool_id.
    ...
    # Prometheus is the thing that broke, or has not scraped ceph yet. A
    # capacity alarm that goes quiet exactly when the metrics stack is down is
    # worse than a second read, so fall back to asking ceph directly.
...
# Emits a summed scope=vm row for the GUI, a scope=fs row per filesystem for
# the alarm (a full 10G / beside an empty 500G /data sums to ~2%), and a
# scope=vol row per cinder volume. The volume rollup is keyed off the uuid nova
# writes into the disk's libvirt <serial>, which the agent hands back on each
# filesystem's disk[].serial -- the only thing that ties a guest mountpoint to
# a volume. An ephemeral root disk carries no serial and so has no vol row.
...
storage_usage_nfs_disks()
{
    ...
    for name in $(echo "$storages" | jq -r '.[] | select(.driver | test("nfs\\.NfsDriver$")) | .name' 2>/dev/null) ; do
        ...
        # cinder_get_storage fails outright when a backend has no extra-config
        # ownership file -- one written by the storage API, so a backend created
        # any other way has none and the whole backend would go unreported with
        # nothing said. The conf is the same source that call reads.
...
# Labels follow the per-instance convention the rest of the fleet uses --
# resource / project / project_name / name -- so these series bind to the
# Instance dashboard's $UUID variable and sit beside ceilometer_* and
# cube_instance_*. Naming them instance_id/vm_name would orphan them.
#
# The Prometheus half of the collection. Grafana reads Prometheus, not influx --
# the influx line protocol exists only because Kapacitor cannot read Prometheus
# and the alarm needs a stream. Same records, two outputs, one collection.
# Written to a .tmp and renamed: node_exporter reads the directory on its own
# schedule and would otherwise serve a half-written file.
_storage_usage_textfile()
{
    ...
    jq -r '
      ...
      . as $r |
      # A ceph pool is already exported as ceph_pool_* by ceph-mgr; re-emitting
      # it here would give the same numbers two names. Non-ceph backends have no
      # exporter at all, so they still need these.
...
# Telegraf exec entry point. One writer per cluster; skips a run already in progress.
...
storage_usage_collect()
{
    ...
    # The lock holds a pid, because a run killed before it can clean up -- a
    # telegraf timeout, a SIGPIPE from a piped read, any SIGKILL -- would
    # otherwise leave the directory behind and silence the sampler for good.
    # An empty or dead pid means the holder is gone, so take it over.
    ...
    # The VM alert template streams this from telegraf.def. One point per thing
    # that can fill up, carrying the best reading available for it: the guest
    # filesystem where the agent answers, the block layer where it does not.
    # Without the fallback an instance with no agent raises no disk alarm at
    # all -- it simply never appears, and nothing says so.
    #
    # Block rows are emitted only for instances with no guest reading at all,
    # never beside one: the two measure different things, and a disk reported
    # twice would alarm twice. `source` says which layer answered; a block row
    # carries its disk in mountpoint, the field the alarm groups and names.
    vmfs=$(jq -c -n \
...
{
    # settings.txt records only what was changed from the default, so each
    # fallback below is the default hex_config declares in config_cinder.cpp.
    # Two copies of one number, but the alternative is a file of our own that
    # hex_config writes and this sources, which is a second mechanism for
    # something the tuning system already does.
