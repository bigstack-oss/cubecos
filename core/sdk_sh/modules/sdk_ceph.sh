    ...
    while [ $i -lt $timeout ] ; do
        if ! curl -k https:/$shared_id:$port/ceph/auth/saml2/metadata 2>/dev/null | grep -q "entityID.*$shared_id" ; then
            ...
            Quiet -n $TERRAFORM_CUBE apply \
                -auto-approve -target=module.keycloak_ceph_dashboard \
...
ceph_device_tier_update()
{
    ...
    # `osd out` was written for, and it is worse off with it, not better. Both
    # ways end at the same CRUSH state; what differs is how many states they
    # pass through, and every step between them moves data. Computed with
    # crushtool against this cluster's own map, 128 PGs on such a rule:
    #
    #   leaving the OSDs in:  71 PGs remap, in one step
    #   marking them out:     51 (out) + 50 (class change) + 0 (in) = 101
    #
    # The last step is free only because the OSD has left the class by then --
    # which is the whole reason marking it out bought nothing. The OSD has to
    # drain off a class-scoped pool when it leaves that class either way.
    #
