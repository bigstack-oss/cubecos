...
migrate_keystone_db()
{
    ...
    if is_control_node; then
        ...
        local id_url=$(/usr/bin/mysql -u root -D keystone -e "SELECT id,url from endpoint where interface='admin'" | awk '/35357/{print $1" "$2}')
        local id=$(echo $id_url | awk '{print $1}')
        local url=$(echo $id_url | awk '{print $2}'| sed 's/35357/5000/g')
        /usr/bin/mysql -u root -D keystone -e "UPDATE endpoint set url='$url' where id='$id'"
    fi
...
migrate_keystone()
{
    ...
    if ! echo $assign | grep -q $(os_get_user_id_by_name admin); then
        openstack role add --user admin --domain default admin
    fi
    if ! echo $assign | grep -q $(os_get_user_id_by_name admin_cli); then
...
migrate_neutron_db()
{
    ...
    if is_control_node; then
        ...
        /usr/bin/mysql -u root -D neutron -e "SET FOREIGN_KEY_CHECKS = 0 ; TRUNCATE TABLE agents ; SET FOREIGN_KEY_CHECKS = 1"
        /usr/bin/mysql -u root -D neutron -e "UPDATE ml2_port_binding_levels SET driver='ovn' WHERE driver='linuxbridge'"
        /usr/bin/mysql -u root -D neutron -e "UPDATE ml2_port_bindings SET vif_type='ovs' WHERE vif_type='bridge'"
        /usr/bin/mysql -u root -D neutron -e "UPDATE networksegments SET network_type='geneve' WHERE network_type='vxlan'"
        /usr/bin/mysql -u root -D neutron -e "TRUNCATE TABLE networkdhcpagentbindings"
        /usr/bin/mysql -u root -D neutron -e "TRUNCATE TABLE routerl3agentbindings"
...
migrate_nova_db()
{
    ...
    if is_control_node; then
        ...
        if [[ "x$(openstack --version)" =~ "xopenstack 6." ]]; then
...
migrate_nova_db_post()
{
    ...
    if is_control_node; then
        ...
        os_nova_service_remove $HOSTNAME "nova-consoleauth"
    fi
...
migrate_ceph()
{
    ...
    if [ ! -e $CEPHFS_CLIENT_AUTHKEY ]; then
        ceph-authtool -p $ADMIN_KEYRING > $CEPHFS_CLIENT_AUTHKEY 2>/dev/null
