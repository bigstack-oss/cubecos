...
{
    for t in $($NVIDIA_SMI vgpu -s -v | grep "vGPU Type ID" | awk '{print $5}' | sort | uniq)
    do
...
gpu_vm_stats()
{
    ...
    for v in $(echo "$stats" | grep "VM UUID" | awk '{print $4}')
    do
        if [ -z "$v" ]; then
            continue
...
gpu_host_stats()
{
    ...
    for p in $(echo "$stats" | grep "^GPU" | awk '{print $2}')
    do
        if [ -z "$p" ]; then
            continue
