...
{
    for N in $(cubectl node list -r compute -j | jq -r .[].hostname); do
        echo "${N}(`os_get_instance_by_node $N | wc -l`)"
    done
}
...
os_instance_ha_helper()
{
    ...
    if [ ! -e $VIP ]; then
        timeout $SRVSTO hex_sdk health_vip_report > $VIP
    else
        OLD_VIP=$(cat $VIP)
        NEW_VIP=$(timeout $SRVSTO hex_sdk health_vip_report)
...
os_image_list()
{
    ...
    local o="{"
    for id in $(echo "$projects" | awk '{print $1}')
    do
...
os_extpack_image_mount()
{
    ...
    for EXT in $(ls ${DIR}/*.ext 2>/dev/null); do
...
/**
* Lists all image files in the specified directory that match the given prefix and file extensions.
*
* @param {string} [DIR=$CEPHFS_GLANCE_DIR] - The directory to search for image files. Defaults to $CEPHFS_GLANCE_DIR if not provided.
* @param {string} PREFIX - The prefix to match in the image file names.
...
os_s3_cmd_by_name()
{
    local user=$1
    shift 1
    local keys=$(openstack ec2 credentials list --user $user -c Access -c Secret -f value | head -n 1)
...
os_s3_list()
{
    local user=$1
    local path=$2
    os_s3_cmd_by_name $user ls s3://$path
}
...
os_s3_bucket_create()
{
    local user=$1
    local path=$2
    os_s3_cmd_by_name $user mb s3://$path
}
...
os_s3_bucket_remove()
{
    local user=$1
    local path=$2
    os_s3_cmd_by_name $user rb s3://$path
}
...
EOF
    os_s3_cmd_by_name $user setpolicy $s3_policy s3://$path
...
os_s3_bucket_info()
{
    local user=$1
    local path=$2
    os_s3_cmd_by_name $user info s3://$path
}
...
os_s3_object_put()
{
    ...
    local file=$2
    local path=$3
    os_s3_cmd_by_name $user put $file s3://$path
}
...
os_s3_object_get()
{
    ...
    local path=$2
    local file=$3
    os_s3_cmd_by_name $user get s3://$path $file
}
...
os_s3_object_delete()
{
    local user=$1
    local path=$2
    os_s3_cmd_by_name $user del s3://$path
}
...
os_ironic_config_sync()
{
    ...
    if [ -n "$flat" -a "$flat" != "null" ]; then
        ...
        if [ -n "$subnet" -a "$subnet" != "null" ]; then
            ...
            for mac in $(echo "$ports" | awk '{print $2}')
            do
...
os_instance_export_list()
{
    ...
    local o="["
    for e in $(cd $DIR && ls -lh export-*.tar | awk '{print $9","$5}')
    do
...
os_instance_convert_image_stat()
{
    ...
    if [ $CHECK_STATUS = "active" ]; then
        ...
        read -p "." -t 10
        local CHECK_STATUS=$(openstack image show $IMAGE_ID -f value -c status)
...
os_mgr_port_clear()
{
    ...
    for n in $(echo "$stats" | awk '{print $2}' | sort | uniq)
    do
...
os_nova_instance_ping()
{
    ...
    for cmp in $(cubectl node -r compute list -j | jq -r .[].hostname)
    do
    ...
    do
        ...
        for ip in $(echo "$stats" | grep $n | awk '{print $4}')
        do
...
os_device_profile_create()
{
    ...
    OLDIFS="$IFS"
    IFS=$'\n'
    for d in $(openstack accelerator device list -f value -c vendor -c std_board_info | sort | uniq)
    do
...
{
    for fid in $(openstack flavor list --long --all -f value -c ID -c Properties | grep "accel:device_profile" | awk '{print $1}')
    do
...
os_nova_pgpu_host_list_by_instance_id()
{
    ...
    for rp in $(openstack allocation candidate list --resource PGPU=1 -f value -c "resource provider" -c traits | grep "$gvendor.*$gid" | awk '{print $1}' | sort)
    do
...
os_instance_pgpu_migrate()
{
    ...
    for arq in $(echo "$arqs_state" | awk '{print $1}')
    do
