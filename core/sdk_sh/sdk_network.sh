...
network_host_ping()
{
    for h in $(cubectl node list | awk -F',' '{print $1}')
    do
...
network_device_ping()
{
    ...
    for ping in $(cat $DEV_LIST | grep -v "role=cube" | awk -F"ping=" '{print $2}' | awk -F',' '{print $1}')
    do
        ping=$(echo $ping | tr -d '\n' | tr -d '\r')
        if [ -n "$ping" ]; then
    ...
    for host in $(cat $DEV_LIST | grep -v "role=cube" | awk -F"hostname=" '{print $2}' | awk -F',' '{print $1}')
    do
        host=$(echo $host | tr -d '\n' | tr -d '\r')
        if [ -n "$host" ]; then
