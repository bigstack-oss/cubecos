...
InstanceHaConfigureMain(int argc, const char** argv)
{
    ...
    if (actIdx == ACTION_ADD) {
        ...
        if (!CliReadInputStr(argc, argv, 2, LABEL_SEG_NAME, &segment) || segment.length() == 0) {
        ...
        if (!CliReadInputStr(argc, argv, 3, LABEL_HOST_LIST, &hostgroup) || hostgroup.length() == 0) {
...
CLI_MODE(CLI_TOP_COMMAND_IAAS, "compute",
...
CLI_MODE(CLI_TOP_COMMAND_IAAS, "instance_ha",
