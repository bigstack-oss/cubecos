...
UpdateCfg(const std::string& domain, const std::string& userPass, const std::string& plaPass, const std::string& novaPass)
{
    if(IsControl(s_eCubeRole) || IsCompute(s_eCubeRole)) {
        ...
        // /opt/openstack-antelope/bin/python3.10. cinder, neutron, nova and manila
        // pin the caracal path for the same reason, and glance, which has moved on to
        // the epoxy venv, pins that venv's helper.
        //
