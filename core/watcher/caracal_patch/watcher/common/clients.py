...
class OpenStackClients(object):
    ...
    def monasca(self):
        ...
        auth = loader.load_from_options(
            ...
            project_name=watcher_clients_auth_config.project_name,
            user_domain_name=watcher_clients_auth_config.user_domain_name,
            project_domain_name=watcher_clients_auth_config.project_domain_name,
        )

        sess = session.Session(auth=auth)
        endpoint = self.session.get_endpoint(service_type='monitoring',
                                             interface=monascaclient_interface,
                                             region_name=monascaclient_region)
        self._monasca = monclient.Client(monascaclient_version, session=sess, endpoint=endpoint)
