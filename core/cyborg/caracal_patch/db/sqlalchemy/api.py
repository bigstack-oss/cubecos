...
def model_query(context, model, *args, **kwargs):
    ...
    """
    # NOTE(cube): the query is returned after this scope has exited, so a caller
    # that consumes it lazily is left holding a connection that no scope owns any
    # more. It is finally reset from SQLAlchemy's _finalize_fairy weakref callback,
    # and under eventlet that runs in the hub, where the rollback's socket read
    # raises "do not call blocking functions from the mainloop" (sometimes 2006 or
    # 2013 instead, once the socket is already dead). enginefacade scopes are
    # re-entrant, so the callers on cyborg's report_data path open their own
    # _session_for_read() and this one nests inside it rather than releasing the
    # connection early. Fixed upstream by taking context.session instead, as part
    # of the SQLAlchemy 2.0 refactor, which is too invasive to backport here.
...
class Connection(api.Connection):
    ...
    def attach_handle_get_by_filters(self, context,
        ...
        """
        # NOTE(cube): hold the read scope, see model_query()
