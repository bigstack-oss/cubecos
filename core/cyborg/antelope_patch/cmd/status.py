...
# NOTE(cube): importing authorize_wsgi is what registers the oslo.policy
# options (it calls oslo_policy.opts.set_defaults at import time, with
# cyborg's own default policy file). The status tool never imports the API
# stack, so without this CONF.oslo_policy does not exist and
# _check_policy_json below dies with:
#     NoSuchOptError: no such option oslo_policy in group [DEFAULT]
from cyborg.common import authorize_wsgi  # noqa: F401
