...
rootfs_install::
	...
	$(Q)for i in {1..3} ; do timeout 120 git clone -b release_3.1.20 --depth 1 https://github.com/bigstack-oss/skyline-apiserver.git $(ROOTDIR)/skyline-apiserver && break ; done
...
heavyfs_install::
	...
	$(Q)chroot $(ROOTDIR) $(SKYLINE_PIP) uninstall -y skyline-apiserver
	$(Q)chroot $(ROOTDIR) $(SKYLINE_PIP) cache remove skyline-apiserver
	$(Q)for i in {1..3} ; do timeout 120 git clone -b v3.1.20-rc1 --depth 1 https://github.com/bigstack-oss/skyline-apiserver.git $(ROOTDIR)/skyline-apiserver && break ; done
	$(Q)chroot $(ROOTDIR) sh -c "cd /skyline-apiserver && $(SKYLINE_PIP) install $(SKYLINE_PIP_C) -r requirements.txt && $(SKYLINE_PIP) install $(SKYLINE_PIP_C) ."
	$(Q)cp ${ROOTDIR}/skyline-apiserver/etc/gunicorn.py ${ROOTDIR}/etc/skyline/gunicorn.py
	$(Q)$(INSTALL_DATA) $(ROOTDIR) $(COREDIR)/skyline/skyline-apiserver.service ./lib/systemd/system
	$(Q)$(INSTALL_DATA) -f $(ROOTDIR) $(COREDIR)/skyline/skyline.yaml.in .$(SKYLINE_CONF_DIR)/skyline.yaml.in
...
rootfs_install::
	...
	$(Q)for i in {1..3} ; do timeout 120 git clone -b release_3.1.20 --depth 1 https://github.com/bigstack-oss/skyline-console.git $(ROOTDIR)/skyline-console && break ; done
	$(Q)# enable nvm
