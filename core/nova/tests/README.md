...
Against a node's epoxy venv, where nova is already importable:

```bash
scp test_cube_live_resize.py root@<node>:/tmp/
ssh root@<node> 'cd /tmp && /opt/openstack-epoxy/bin/python -m unittest test_cube_live_resize -v'
```

Or in a local venv built the way the port was verified:

```bash
python3.12 -m venv /tmp/nova-test && . /tmp/nova-test/bin/activate
pip install -c core/heavyfs/os-epoxy-pip-upper-constraints.txt nova==31.3.1
# apply core/nova/epoxy_patch over site-packages/nova, then run unittest
```

The constraints file matters: without it the venv picks up an oslo.utils / setuptools
newer than epoxy expects and nova will not import.
