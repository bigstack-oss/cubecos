...
def _get_service_user_auth():
    ...
    resume-on-host-boot path: nova-compute works there from an admin
    context it creates itself. service_auth.get_auth_plugin() cannot
    serve that case -- it wraps a tokenless user auth, and
    keystoneauth then fails while assembling the request headers.
    """
