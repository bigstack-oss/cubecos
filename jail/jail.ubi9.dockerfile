...
# literal "hex" used to be rewritten by systemd's %post, but the stream9 base image
# now ships systemd, so that transaction -- and the accident -- is gone. The fallback
# covers a base image without systemd: an empty machine-id is regenerated by that
# same %post once the tier2 install pulls systemd in.
RUN systemd-machine-id-setup || : > /etc/machine-id
